...
#these 2 lines help us to debug
import cgitb
cgitb.enable() 
...
def print_guess_stats(word, guess):
    for i, c in enumerate(guess): #enumetate takes the string, it loops over each charactor one by one, takes the first charactor e.g. w, put it in c and take the location of it and put it in i
        if c == word[i]:
            put_box('green') #correct place
        elif c in word:
            put_box('yellow') #incorrect place
        else:
            put_box('red') # not right letter


    if word == guess:
        print('<div> Yay! You got it in ' + str(guess_num) + ' guesses! </div>')
